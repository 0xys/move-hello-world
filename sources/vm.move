module 0x3::Vm {
    ...
    struct Account has key { balance: u128, code: vector<u8> }

    public fun init(account: &signer) {
        move_to(account, Account { balance: 0, code: vector::empty<u8>() });
    }
    ...
    public fun call(addr: address, calldata: vector<u8>): vector<u8> acquires Account {
    ...
    public fun unpublish(account: &signer) acquires Account {
        let Account { balance: _, code: _ } = move_from<Account>(signer::address_of(account));
    }
}